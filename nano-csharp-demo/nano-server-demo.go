...
type (
	...
	// UserLeft message will be received when one user left room 
	UserLeft struct {
		Content string `json:"content"`
	}
...
func (mgr *RoomManager) AfterInit() {
	session.Lifetime.OnClosed(func(s *session.Session) {
		...
		if err != nil {
			println(fmt.Sprintf("remove user [%d] failed", s.ID()))
		} else {
			room.group.Broadcast("onUserLeft", &UserLeft{Content: fmt.Sprintf("user [%d] left", s.ID())})
		}
...
func main() {
	...
	nano.Listen(":3250", nano.WithPipeline(pipeline))
}
